...
for m = 1:length(zRange)        
    ...
    z = zRange(m);    
    %x-axis value of the receiver    x_r = 0;
    
    %Define the integrand E_r(x,y) in (31) and its absolute value square (we have removed E_i/(4*pi*rho)*exp(-1j*2*pi*rho/lambda) since it will cancel out)
...
plot(relativeRange, RISGainFarfield,'r-.', 'Linewidth', 2);set(gca,'XScale','log');
xticks([10 100 1000 10000 100000]); xticklabels({'$10 d_F$','$10^2 d_F$','$10^3 d_F$','$10^4 d_F$','$10^5 d_F$'})
...
xlabel('Propagation distance ($z$)','Interpreter','Latex');ylabel('Normalized RIS gain','Interpreter','Latex');set(gca,'fontsize',18);