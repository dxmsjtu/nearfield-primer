...
%Wavelength
lambda = 3e8/f_c;
%Changing this factor will not affect the result since everything becomes scaled accordingly
scalefactor = 1/4;
...
for m = 1:length(zRange)        
    ...
    %Define the integrand described in the text on Page 4 and its absolute value square (we have removed E_0/sqrt(4*pi) since it will cancel out)
...
set(gca,'XScale','log');xticks([1 10 100 1000]);xticklabels({'$d_F$','$10 d_F$','$100 d_F$','$1000 d_F$'})
ylim([0 1]);legend({'Scalar-field approximation','Upper bound','Exact'},'Interpreter','Latex','Location','SouthEast');
