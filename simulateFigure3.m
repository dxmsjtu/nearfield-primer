...
%Emil Björnson, Özlem Tuğfe Demir, and Luca Sanguinetti, "A Primer on Near-Field Beamforming for Arrays and Reconfigurable 
%Intelligent Surfaces,"  Asilomar Conference on Signals, Systems, andComputers, Virtual conference, October-November 2021.
...
%Wavelength
lambda = 3e8/f_c;
%This factor can be used to change the antenna size. We need to have a value above 0.6 to satisfy the condition d_F >= 1.2D
scalefactor = 2;
...
for n = 1:length(zRange)    
    ...
    z = zRange(n);    
    %Define the integrand E(x,y) in (5) and its absolute value square (we have removed E_0/sqrt(4*pi) since it will cancel out)
...
figure;hold on; box on; grid on;plot(zRange/fraunhoferDistance, G_exact,'k-', 'Linewidth', 2);plot(zRange/fraunhoferDistance, G_approx,'r-.', 'Linewidth', 2);
set(gca,'XScale','log');xticks([0.1 0.3 1 10]); xticklabels({'$0.1 d_F$','$0.3 d_F$','$d_F$','$10 d_F$'})
ylim([0 1]);legend({'Exact', 'Fresnel approximation'},'Interpreter','Latex','Location','SouthEast');
xlabel('Propagation distance ($z$)','Interpreter','Latex');ylabel('Normalized antenna gain','Interpreter','Latex');set(gca,'fontsize',18);
%
